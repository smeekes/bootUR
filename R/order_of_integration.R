...
#      warning("'iADFtest' is deprecated. Use 'boot_ur' instead.")
      out <- iADFtest(datad, ...)
    } else if (method == "bFDRtest" & N > 1) {
#      warning("'bFDRtest' is deprecated. Use 'boot_fdr' instead.")
      out <- bFDRtest(datad, ...)
    } else if (method == "BSQTtest" & N > 1) {
#      warning("'BSQTtest' is deprecated. Use 'boot_sqt' instead.")
      out <- BSQTtest(datad, ...)
    } else if (method == "boot_df" & N == 1) {
#      warning("'boot_df' is deprecated. Use 'boot_adf' instead.")
      out <- boot_df(datad, ...)
    } else {
      stop("Invalid 'method' argument.")
    }
    d[i_in_datad[!out$rej_H0]] <- d_i + 1
    if (any(out$rej_H0)) {
      if (d_i == 0) {
