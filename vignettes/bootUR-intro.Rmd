...
```{r union}
union_out <- boot_ur(GDP_NL, B = 399, bootstrap = "SWB", verbose = TRUE)
```
...
panel_out <- boot_panel(MacroTS, bootstrap = "MBB", B = 399, verbose = TRUE)
```
...
```{r panel}
panel_out <- boot_panel(MacroTS, bootstrap = "DWB", B = 399, verbose = TRUE)
```
...
```{r iADF}
ADFtests_out <- boot_ur(MacroTS[, 1:5], bootstrap = "MBB", B = 399, verbose = TRUE, union = FALSE, 
...
BSQT_out1 <- boot_sqt(MacroTS, steps = 0:N, bootstrap = "AWB", B = 399, verbose = TRUE)
# Split in four equally sized groups (motivated by the 4 series per country)
BSQT_out2 <- boot_sqt(MacroTS, steps = 0:4 / 4, bootstrap = "AWB", B = 399, verbose = TRUE)
```
...
```{r bFDR}
N <- ncol(MacroTS)
FDR_out <- boot_fdr(MacroTS[, 1:10], level = 0.1, bootstrap = "BWB", B = 399, verbose = TRUE)
```
